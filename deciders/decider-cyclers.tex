...
\caption{Space-time diagrams of the 30 first steps of bbchallenge's machines \#279,081 and \#4,239,083 which are both ``Cyclers'': they eventually repeat the same configuration for ever. Please refer to \url{https://bbchallenge/279081} and 
\url{https://bbchallenge/4239083} for the transition tables.}\label{fig:cyclers}
\end{figure}
...
\begin{example}\normalfont
Figure~\ref{fig:cyclers} gives the space-time diagrams of the 30 first iterations of two ``Cyclers'' machines: bbchallenge's machines \#279,081 and \#4,239,083. Refer to \url{https://bbchallenge/279081} and 
...
\begin{algorithm}
        ...
        \begin{algorithmic}[1]
                ...
                \State \tabi\textbf{int $\boldsymbol{\vdash}$ int} tape
                \State \}
...
\begin{theorem}\label{th:main}\normalfont Let $\mathcal{M}$ be a Turing machine and $t \in \mathbb{N}$ a time limit. Let $c_0$ be the initial configuration of the machine. There exists $i\in\mathbb{N}$ and $j\in\mathbb{N}$ such that $c_0 \vdash^i c_i \vdash^j c_i$ with $i+j \leq t$ if and only if {\sc decider-cyclers}($\mathcal{M}$,$t$) returns true.
\end{theorem}
\begin{proof}
...
\end{corollary}
\begin{proof}
        By Theorem~\ref{th:main}, there exists $i\in\mathbb{N}$ and $j\in\mathbb{N}$ such that $c_0 \vdash^i c_i \vdash^j c_i$ with $i+j \leq t$. It follows that for all $k\in\mathbb{N}$, $c_0 \vdash^{i+kj} c_i$. The machine never halts as it will visit $c_i$ infinitely often.
\end{proof}
