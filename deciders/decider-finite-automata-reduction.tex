...
This idea has been explored by other authors under the name Closed Tape Languages (CTL) as described in S. Ligocki's blog \cite{ShawnCTL} and credited to H. Marxen in collaboration with J. Buntrock. \ts{For a given Turing machine, the CTL technique consists in trying to construct a set $C$ of configurations such that:

  \begin{enumerate}
    \item Starting from the all-0 initial configuration, the TM eventually reaches a configuration $c \in C$.
    \item $C$ is \textit{closed}: for any $c \in C$, a positive number of TM steps from $c$ leads to $c' \in C$
    \item $C$ does not contain any halting configuration
  \end{enumerate}

  If such a set $C$ exists then the machine does not halt. The CTL approach has proven to be practical and powerful when we search for $C$ among regular languages \cite{ShawnCTL} \cite{BruteforceCTL}.
...
Here, we develop an original \ts{\textit{co-CTL}} technique\footnote{\ts{By co-CTL we mean the complement of a CTL language, i.e. recognising eventually-halting configurations and not including the initial one. The CTL and co-CTL approaches are equivalent, they correspond the contraposition of one another.}}, based on the algebraic description of Nondeterministic Finite Automata (NFA), for finding a regular language associated to a machine's eventually halting configurations.
