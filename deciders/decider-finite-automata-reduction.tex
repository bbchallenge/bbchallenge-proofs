...
For a given Turing machine, we aim at building a finite-state machine that recognises at least all eventually-halting configurations. That way, if it does not recognise the initial configuration then we know that the Turing machine does not halt from it.
...
\ts{Let $\M_{m,n}$ be the set of matrices with $m$ rows and $n$ columns over the Boolean domain $\mathbf{2} = \{0,1\}$ where operations $+$ and $\cdot$ are respectively implemented by $\operatorname{OR}$ and $\operatorname{AND}$.}

We may define a Nondeterministic Finite Automaton (\textbf{NFA}) with $n$ states and alphabet $\mathcal{A}$ \ts{as a tuple $(q_0, \{T_\gamma \; | \; \gamma \in \mathcal{A}\}, a)$} where $q_0 \in \M_{1,n}$ and $a \in \M_{n,1}$ respectively represent the initial states and accepting states of the NFA \ts{(i.e. if the $i^\text{th}$ state of the NFA is an initial state then the $i^\text{th}$ entry of $q_0$ is set to 1, and if the $i^\text{th}$ state of the NFA is accepting then the $i^\text{th}$ entry of $a$ is set to 1)}, and where transitions are matrices $T_\gamma\in \M_{n,n}$ for each \tss{$\gamma\in\{0,1,A,\ldots,E\}$}{$\gamma\in\mathcal{A}$}. A word $u=\gamma_1\dots\gamma_\ell \in \mathcal{A}^*$ \ts{is accepted by the NFA iff one of its execution traces by the NFA reaches an accepting state}, which algebraically translates to $q_0 T_u a = 1$. \todo{JEB: Citation needed? \href{https://en.wikipedia.org/wiki/Two-element_Boolean_algebra}{LINK}}
...
We wish to describe Turing machine configurations with finite words, which will be read by our NFAs.
Since we only work with configurations that have finitely many nonzero symbols on the tape,
we can transform a Turing machine configuration into a word in the alphabet $\{0,1,A,B,C,D,E\}$
that will describe any segment of the machine's tape that contains at least all nonzero cells and write the head's state before the bit it's on.
For instance, the word associated to the configuration depicted in Figure~\ref{fig:far_scan} is \texttt{00A001100}. \ts{We call \texttt{00A001100} a \textit{finite word-representation} of the configuration depicted in Figure~\ref{fig:far_scan}. Note that, since we work with configurations that have finite support, we can add arbitrarily many leading or trailing 0s to \texttt{00A001100} without changing the configuration that it is representing. Hence, if we call $\mathcal{L}$ the regular language of the NFA that we wish to construct to recognise the eventually-halting configurations of a given TM, it is natural that we ask the following:
...
\ts{With $c, c_1, c_2$ configurations of the TM (with finite support) and $\hat{c}, \hat{c}_1, \hat{c}_2$ any of their finite word-representations as described above. Let $f,t \in \{A,B,C,D,E\}$ denote TM states (the ``from'' and ``to'' states in a TM transition), and $r,w,b \in \{0,1\}$ denote bits (a bit ``read'', a bit ``written'', and just a bit), then the above conditions turn into:}
...
For \tss{left/right rules}{machine transitions going left/right}, simply require $T_t T_b T_w\preceq T_b T_f T_r$ and $T_w T_t\preceq T_f T_r$, respectively.

To simplify \tss{the halt-rule condition}{condition for halting machine transitions}:
assert \ts{that for all $u \in \mathbf{2}^*$}, the sub-expression $q_0 T_{u}$ is \tss{never}{not} zero \ts{(this means that reading 0 and 1 from the initial states must always lead to some state\footnote{This assertion is not limiting since if it is not met by some NFA, adding an additional sink state to the NFA will not change the recognised language and will satisfy the assertion.})},
...
Going further: define an \emph{accepted steady state}\tsm{I have an issue with using the word state for \textit{steady state} here, since $s$ is a set of NFA states. This issue of using the word state to mean a set of states arises at other points of the text.} $s$ to be a \ts{row} vector such that $sa = 1$, $s T_0\succeq s$, and $s T_1\succeq s$.
Given such $s$ \ts{we have}, $q'\succeq s\implies \forall z\in\mathbf{2}^*: q'T_{z}a = 1$.

We have shown:

\begin{theorem}\normalfont
  \label{far-main-theorem}
  Machine $\mathcal{M}$ doesn't halt if there are an NFA $(q_0, \{T_\gamma\}, a)$ and \ts{row} vector $s$ satisfying the below:
  \begin{align}
