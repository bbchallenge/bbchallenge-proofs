...
For instance, the word associated to the configuration depicted in Figure~\ref{fig:far_scan} is \texttt{00A001100}.

\ts{Let $\mathcal{L}$ be the regular language recognised by the NFA that we wish to construct for a given TM. Because we work with TM configurations that have finite support, as many leading and trailing 0s can be added to the finite word-representation of a configuration without changing the configuration that it is representing. Hence, it is natural that we ask the following for $\mathcal{L}$:
...
\ts{With $c, c_1, c_2$ configurations of the TM (with finite support) and $\hat{c}, \hat{c}_1, \hat{c}_2$ any of their associated finite words as described above. Let $f,t \in \{A,B,C,D,E\}$ denote TM states (the ``from'' and ``to'' states in a TM transition), and $r,w,b \in \{0,1\}$ denote bits (a bit ``read'', a bit ``written'', and just a bit), then the above conditions turn into:}
