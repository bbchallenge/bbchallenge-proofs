...
\begin{center}
  ...
  \end{tikzpicture}
  \end{center}
  \caption{Example Nondeterministic Finite Automaton (NFA) with 3 states (X,Y,Z), alphabet $\mathcal{A} = \{\alpha,\beta\}$, initial states X and Y, and accepting states Y and Z. The linear-algebra representation of this NFA is given in Example~\ref{ex:nfa}. Example accepted words are: $\alpha$, $\alpha\beta$, $\alpha\alpha\beta\beta$. Example rejected words are: $\beta$, $\beta\alpha$, $\beta\beta\alpha\beta$.}\label{fig:example_nfa}
\end{figure}
\begin{example}\label{ex:nfa}\normalfont
The NFA depicted in Figure~\ref{fig:example_nfa} is algebraically encoded as follows: $q_0 = (1,1,0)$, $a=(0,1,1)$
\end{example}
