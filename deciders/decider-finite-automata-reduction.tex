...
\ts{With $c, c_1, c_2$ configurations of the TM with finite support and $\hat{c}, \hat{c}_1, \hat{c}_2$ their associated finite word as described above. Algebraically, it turns into, with $f,t \in \{A,B,C,D,E\}$ denoting TM states (the ``from'' and ``to'' states in a TM transition), and $r,w,b \in \{0,1\}$ denoting bits (a bit ``read'', a bit ``written'', and just a bit):}
