...
We now seek criteria sufficient to preclude false negatives, \ts{i.e. avoiding that the finite-state machine would miss some eventually-halting configurations}.

We may define a Nondeterministic Finite Automaton (\textbf{NFA}) with $n$ states in linear-algebraic terms:
...
This is most natural with matrices over the Boolean domain $\mathbf{2}$, with operations $\operatorname{OR}$ and $\operatorname{AND}$ respectively for $+$ and $\cdot$.
...
We wish to apply an NFA to TM configurations.
...
We only work with configurations that have finitely many nonzero symbols on the tape. Define the \textit{support of a configuration} to be any finite tape segment that includes all the nonzero bits of the configuration. 
We form words by concatenating the tape symbols of the support and writing the head's state before the bit it's on. For instance, the word associated to the tape depicted in Figure~\ref{fig:far_scan} reads \texttt{00A001100}.}
Acceptance condition $q_0 T_u a\ne 0$ is well-defined, provided that:
\begin{align}
  q_0 T_0 &= q_0
