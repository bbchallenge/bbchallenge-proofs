...
We call \texttt{00A001100} a \textit{finite word-representation} of the configuration depicted in Figure~\ref{fig:far_scan}. Note that, since we work with configurations that have finite support, we can add arbitrarily many leading or trailing 0s to \texttt{00A001100} without changing the configuration that it is representing. Hence, if $\mathcal{L}$ is the regular language of the NFA that we wish to construct to recognise the eventually-halting configurations of a given TM, it is natural that we ask the following:
