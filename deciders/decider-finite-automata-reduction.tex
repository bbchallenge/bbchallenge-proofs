...
As a generalization of backward reasoning, we can try to recognize eventually-halting configurations using finite automata, as in figure \ref{fig:finite-automata-reduction}\tsm{A bit too brief, it is quite vague (even to the informed reader) how "recognizing eventually-halting configurations using finite automata" relates or generalises BR.}.
...
We may define \tss{an NFA}{a Non-deterministic Finite Automaton (\textbf{NFA})} \tss{(with $n$ states)}{with $n$ states} in linear-algebraic terms:
...
This is most natural with matrices over the Boolean domain $\mathbf{2}$, with operations \tss{$+=\operatorname{OR}$ and $\cdot=\operatorname{AND}$}{$\operatorname{OR}$ and $\operatorname{AND}$ respectively for $+$ and $\cdot$ operations}.
...
We wish to apply \tss{the}{an} NFA to TM configurations.
...
Acceptance condition $q_0 T_u a\ne 0$ is \tss{now}{} well-defined, provided that:
\begin{align}
  q_0 T_0 &= q_0
