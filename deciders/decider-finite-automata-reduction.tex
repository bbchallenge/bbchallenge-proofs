...
\item $(q_0,\{L_0, L_1\})$ comes from a DFA with transition function $\delta: [l] \times \{0,1\} \to [l]$ that ignores leading zeros (i.e. the DFA has an edge labelled 0 coming from and to state $q_0$). That ensures \eqref{far-cond-leading-0} of Theorem~\ref{far-main-theorem}.
\item Row vectors of matrices $M_f$ (with $f\in\{A,\ldots,E\}$) are the basis row vectors $e_0,\, \dots,\, e_{5l}$.
\item The right-hand side NFA has a halt state $\bot$ and $e_{5l+1} = e_\bot$ is its corresponding basis row vector.

\end{enumerate}

Theorem~\ref{far-main-theorem} requires that we have a \textit{steady state-set} s (see Section~\ref{far-defs-recognizer}), which we give \textit{ad-hoc}: $s=0\oplus e_\bot$. We seek a ``direct'' algorithm which searches left-hand side DFAs and then finds an associated right-hand side NFA by solving Theorem~\ref{far-main-theorem} \eqref{far-cond-first}--\eqref{far-cond-last} for $a$, $R_0$, $R_1$ if possible.

A helpful property of standard basis vectors is: the condition $\row_i(M) \succeq v$ with $\row_i(M)$ the $i^\text{th}$ row of matrix $M$ and $v$ some row vector, is equivalent to $M\succeq e_i\T v$.
...
\begin{align}
  ...
  \\
  \forall i\in[l]: R_r &\succeq (\row_i(M_f))^{\T} (\row_{\delta(i,w)}(M_t))^{\T}
  &&\text{if $(f,r) \to (t,w,\text{right})$ is a transition of $\mathcal{M}$}
  %\label{far-cond-right}
