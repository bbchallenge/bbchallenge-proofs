...
then set entries to 1 as these 4 inequalities demand until \tss{a fixed point is reached}{$R_0$ and $R_1$ stop changing} \ts{-- inequality \ref{far-cond-left}' is the only one that does not have a constant right-hand side and which requires an iterative process to reach the solution. Now that we have found $R_0$ and $R_1$ we need to find the set of accepting states, given by $\begin{bmatrix}0&a\end{bmatrix}$ with $a \in \M_{1,d}$.}
...
\ts{Conditions \eqref{far-cond-trailing-0}, \eqref{far-cond-ass-accepted} of Theorem \ref{far-main-theorem}   translate to:
  \begin{align}
    a R_0 & = a
          &
    \tag{\ref{far-cond-trailing-0}'}
  ...
  Similarly, there is a unique minimal (w.r.t $\preceq$) solution to this system which is found by initially setting $a_0 = e_\bot$ then iterating $a_{k+1} = a_{k} R_0$ until a fixed point is reached which gives the value of $a$, satisfying (\ref{far-cond-trailing-0}') and (\ref{far-cond-ass-accepted}'). Indeed, from (\ref{far-cond-ass-steady}'), we see that the sequence $e_\bot \preceq e_\bot R_0\preceq e_\bot R_0^2 \preceq\dots$ is increasing -- notice that $R_0 \succeq (e_\bot)^{\T} e_\bot$ is equivalent to $e_\bot R_0 \succeq e_\bot$ and that if $M \preceq M'$ then for all $M''$ (of compatible dimension) we have $M M'' \preceq M' M''$ -- hence it reaches a fixed point, which satisfies (\ref{far-cond-trailing-0}') and (\ref{far-cond-ass-accepted}').
