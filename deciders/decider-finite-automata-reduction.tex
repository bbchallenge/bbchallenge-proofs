...
\ts{Let $\mathcal{L}$ be the regular language recognised by the NFA that we wish to construct for a given TM. Since the support of a configuration can contain as many leading or trailing 0s as wanted, it is natural that we ask the following conditions:
...
\tss{Acceptance condition $q_0 T_u a\ne 0$ is well-defined, provided that}{We translate these into the following stronger conditions on the transition matrix $T_0 \in \M_{n,n}$}:
\begin{align}
  q_0 T_0 &= q_0
...
\ts{With $c, c_1, c_2$ configurations of the TM with finite support and $\hat{c}, \hat{c}_1, \hat{c}_2$ their associated finite word as described above. Let $f,t \in \{A,B,C,D,E\}$ denote TM states (the ``from'' and ``to'' states in a TM transition), and $r,w,b \in \{0,1\}$ denote bits (a bit ``read'', a bit ``written'', and just a bit), then the above conditions turn into:}

\begin{align*}
  \forall u_1,u_2\in\mathbf{2}^*: \; u_1 \, f \, r \, u_2 \in \mathcal{L},\;& \text{if $(f,r) \to \bot$ is a halting transition of $\mathcal{M}$}
  \\
  \forall u_1,u_2\in\mathbf{2}^*: u_1 \,  t\, b\, w \, u_2 \in \mathcal{L} \implies u_1 \, b \, f \, r \, u_2 \in \mathcal{L},\;& \text{if $(f,r) \to (t,w,\text{left})$ is a transition of $\mathcal{M}$}
  \\
  \forall u_1,u_2\in\mathbf{2}^*: q_0 T_{u_1} T_w T_t T_{u_2} a = 1 \implies q_0 T_{u_1} T_f T_r T_{u_2} a = 1,\;& \text{if $\ldots fr\ldots\vdash\ldots wt\ldots$ is a right rule}
\end{align*}

% \begin{align*}
%   \forall u_1,u_2\in\mathbf{2}^*: \; u_1 \, f \, r \, u_2 \in \mathcal{L},\;& \text{if $(f,r)$ is a halting transition of $\mathcal{M}$}
%   \\
