...
\ts{
  \begin{example}\label{ex:nfa}\normalfont
    The NFA depicted in Figure~\ref{fig:example_nfa}, with states X, Y, Z and alphabet $\mathcal{A}=\{\alpha,\beta\}$, is algebraically encoded as follows: $q_0 = (1,1,0)$, $a=(0,1,1)$, $T_\alpha=\begin{bmatrix}
        0 & 0 & 0 \\
        1 & 1 & 0 \\
        0 & 1 & 1
...
\ts{Note that Condition~\ref{far-cond-trailing-0}, $T_0 a^{\T} = a^{\T}$, means that for all accepting states of the NFA, reading a 0 is possible and leads to an accepting state. Indeed, $T_0 a^{\T}$ describes the set of NFA states that reach the set of accepting states $a$ after reading a $0$.}
...
and add more structure to our NFA \ts{in order to design an efficient search algorithm for Theorem~\ref{far-main-theorem}}. \ts{We are going to constrain our NFA in the following way:
  ...
  \begin{enumerate}
    ...
    \item Edges labelled by a tape-head state are only those that start in the left-hand side DFA and end in the right-hand side NFA. Furthermore, we require that no such two edges reach the same state in the right-hand side NFA. Hence, the right-hand side NFA has at least $5l$ states with $l$ the number of states of the left-hand side DFA.
    \item In fact, we require that the right-hand side NFA has exactly $5l+1$ states with the extra state named $\bot$.
...
then set entries to 1 as these 4 inequalities demand until \tss{a fixed point is reached}{$R_0$ and $R_1$ stop changing} \ts{-- inequality \ref{far-cond-left}' is the only one that does not have a constant right-hand side and which requires an iterative process to reach the solution.

  Now that we have found $R_0$ and $R_1$ we need to find the set of accepting states, given by $\begin{bmatrix}0&a\end{bmatrix}$ with $a \in \M_{1,d}$.
  ...
  Similarly, there is a unique minimal (w.r.t $\preceq$) solution to this system which is found by initially setting $a_0 = e_\bot$ then iterating $a_{k+1} = (R_0 a_{k}^{\T})^{\T}$ until a fixed point is reached which gives the value of $a$, satisfying (\ref{far-cond-trailing-0}') and (\ref{far-cond-ass-accepted}'). Indeed, from (\ref{far-cond-ass-steady}'), we see that the sequence $e_\bot^{\T} \preceq  R_0 e_\bot^{\T}\preceq R_0^2 e_\bot^{\T}  \preceq\dots$ is increasing hence it reaches a fixed point, which satisfies (\ref{far-cond-trailing-0}') and (\ref{far-cond-ass-accepted}').
  ...
  % From (\ref{far-cond-ass-steady})', we see that the sequence $e_\bot \preceq e_\bot R_0\preceq e_\bot R_0^2 \preceq\dots$ is increasing -- indeed, notice that $R_0 \succeq (e_\bot)^{\T} e_\bot$ is equivalent to $e_\bot R_0 \succeq e_\bot$ and that is $M \preceq M'$ then for all $M''$ (of compatible dimension) we have $M M'' \preceq M' M''$. Hence, the sequence $(e_\bot R_0^k)_{k\in\mathbb{N}}$ eventually reaches a fixed point, call it $a$, which satisfies (\ref{far-cond-trailing-0}') and (\ref{far-cond-ass-accepted}').
