...
\begin{enumerate}
  ...
  \item The NFA will be constructed from two sub-NFAs: one NFA responsible for handling the left-hand side of the tape (i.e. before reading the tape-head state) and one NFA for handling the right-hand side of the tape (i.e. after reading the tape-head state). 
  \item The sub-NFA for the left-hand side of the tape will be a DFA.
...
This structure might seem arbitrary but it has a very nice property that we demonstrate in this section: once a DFA is chosen for the left-hand side of the tape, there is a unique right-hand side NFA that can be computed in hopes of sastisfying Theorem~\ref{far-main-theorem}. Hence we can use our efficient DFA-search algorithm of Section~\ref{far-defs-dfa} to enumerate left-hand side DFA and for each of them, compute the uniquely associated right-hand side NFA until we find such a pair that fully satisfies Theorem~\ref{far-main-theorem}.
}
