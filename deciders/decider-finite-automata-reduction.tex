...
\item Row vectors of matrices $M_f$ (with $f\in\{A,\ldots,E\}$) are the basis row vectors $e_0,\, \dots,\, e_{5l}$ (basis vector $e_i$ has its $i^\text{th}$ entry set to 1 and the other entries set to 0).\label{pt:basis}
...
then set entries to 1 as these 4 inequalities demand until a fixed point is reached \ts{-- inequality \ref{far-cond-left}' is the only one that does not have a constant right-hand side and which requires iterative search for the solution}. 
