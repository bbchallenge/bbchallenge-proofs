...
The core idea of the method presented in this Section is to find, for a given Turing machine, a regular language that \jbs{describes}{separates} the set \jbs{(or a superset)}{} of the machine's eventually-halting configurations\jbs{. Then, we only have to test that the initial all-0 configuration is not part of the language}{from those which are reachable from its initial all-0 configuration} to deduce that \jb{they are disjoint, and} the machine does not halt \jbs{from it}{}.\jbm{
In general, I liked the old intro better, but we can't just describe co-CTL as our core idea and abruptly define its exact opposite\ldots right?

Maybe we could get away with the original intro followed by ``A dual idea has been explored\ldots''.
}
...
\jb{The CTL technique for proving a Turing machine doesn't halt is to exhibit a set $C$ of configurations such that:

  \begin{enumerate}
    \item $C$ contains the all-0 initial configuration\footnote{\jb{
        Criteria 1--2 give a strict definition; in \cite{ShawnCTL}, $C$ need only contain some descendant of the initial configuration and of the successor to each $c\in C$.
        In that case, the set of ancestor configurations to those in $C$ meets the strict definition; noting this abstractly may be more convenient than constructing this set.
    }}
...
Here, we develop an original \textit{co-CTL} technique\footnote{\jb{By co-CTL we mean a set whose complement is a CTL, characterized by closure criteria inverse---or equivalently converse---to 1--3. In other words, a co-CTL contains all halting configurations, any configuration which can \emph{precede} any member configuration by one TM transition, and not the initial configuration.}}, based on the algebraic description of Nondeterministic Finite Automata (NFA), for finding a regular language \jbs{associated to}{which contains} a machine's eventually halting configurations \jb{(in general a superset)}.
