...
\ts{Let $\mathcal{L}$ be the regular language recognised by the NFA that we wish to construct for a given TM. Because we work with TM configurations with finite support, as many leading and trailing 0s can be added to the finite word-representation of a configuration without changing the configuration that it is representing. Hence, it is natural that we ask the following for $\mathcal{L}$:
