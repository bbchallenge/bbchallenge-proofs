...
This is most natural with matrices over the Boolean domain $\mathbf{2}$, with operations \tss{$+=\operatorname{OR}$ and $\cdot=\operatorname{AND}$}{$\operatorname{OR}$ and $\operatorname{AND}$ respectively for $+$ and $\times$}.
