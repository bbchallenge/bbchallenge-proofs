...
\ts{Note that Condition~\ref{far-cond-trailing-0}, $T_0 a^{\T} = a^{\T}$, asks that for all accepting states of the NFA, reading a 0 is possible and leads to an accepting states. Indeed, $T_0 a^{\T}$ describes the set of NFA states that reach the set of accepting states $a$ after reading a $0$.}
