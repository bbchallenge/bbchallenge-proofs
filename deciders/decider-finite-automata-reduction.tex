...
We may define a Nondeterministic Finite Automaton (\textbf{NFA}) with $n$ states and alphabet $\mathcal{A}$ \ts{as a tuple $(q_0, \{T_\gamma \; | \; \gamma \in \mathcal{A}\}, a)$} where $q_0 \in \M_{1,n}$ and $a \in \M_{n,1}$ respectively represent the initial and accepting states of the NFA, and where transitions are matrices $T_\gamma\in \M_{n,n}$ for each \tss{$\gamma\in\{0,1,A,\ldots,E\}$}{$\gamma\in\mathcal{A}$}. A word $u=\gamma_1\dots\gamma_\ell \in \mathcal{A}^*$ is accepted by the NFA iff one of its execution traces by the NFA reaches an accepting state, which translates to $q_0 T_u a = 1$. \todo{JEB: Citation needed? \href{https://en.wikipedia.org/wiki/Two-element_Boolean_algebra}{LINK}}
