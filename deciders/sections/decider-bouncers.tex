...
We define \textit{record-breaking tapes} to be tapes where the head is pointing at $0^\infty$, i.e.\ tapes of the form $0^\infty u \rhead{s} 0^\infty$ or $0^\infty \lhead{s} u 0^\infty$ with $u\in\Sigma^*$, and, for a given tape $t$, \textit{length} is defined as the number of symbols in $\Sigma$ that it contains (hence, excluding head and $0^\infty$). We denote an infinite sequence $u_0, u_1, \dots u_n, \dots$ with $n\in\N$ by $(u)_{n\in\N}$. We denote the infinite constant integer sequence where all terms equal $K\in\mathbb{N}$ by $(K)_{n\in\mathbb{N}}$. We define the difference operator $D: \N^\N \to \N^\N$ such that $(v)_{n\in\N} = D((u)_{n\in\N})$ if and only if $v_n = u_{n+1}-u_{n}$ for all $n\in\N$.
...
\begin{example}
    ...
    \begin{align*}
        0^\infty \rhead{\text{A}} 0^\infty & \vdash^{37} 0^\infty 111\; 01111C> \rhead{\text{C}} 0^\infty                               & g(0) = 37,\; l_{37} = 11   \\
    ...
    \end{align*}
    Each of these tapes are record-breaking and clearly an element of $\mathcal{L}(f)$. Then, in this finite sample, $l_{g(0)},...,l_{g(3)}$ with first difference $K=5$ and $g(0), ..., g(3)$ is in quadratic progression with second difference $K'=6$.
