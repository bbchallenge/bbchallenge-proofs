...
\begin{theorem}[\textit{Linear-quadratic growth}]
    Let $M$ be a bouncer Turing machine, i.e. it satisfies Theorem~\ref{th:bouncers} for some formula tape $f$. Call $(t_n)_{n\in\N}$ the sequence of tapes that it visits starting from $t_0 = 0^\infty \rhead{\text{A}} 0^\infty$. Call $l_n$ the length of $t_n$ and suppose that $\text{lim}_{n\to\infty} l_n = \infty$, i.e. $M$ is not a cycler. Then, there is $g: \mathbb{N} \to \mathbb{N}$ that extracts a subsequence of $(t_n)_{n\in\N}$ such that:

    \begin{enumerate}
        \item $t_{g(n)}$ is a record-breaking tape for all $n\in\mathbb{N}$, and all tapes $t_{g(n)}$ share the same head state and head pointing-direction.
        \item $t_{g(n)} \in \mathcal{L}(f)$ for all $n\in\mathbb{N}$.
