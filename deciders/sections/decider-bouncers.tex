...
\begin{proof}
    ...
    Concerning Point 3, call $g(n)$ the time step at which $C_{\tilde{f}}(n)$ is reached. By construction, $g(n) \geq n$. The number of macro steps $M \geq 1$ such that we get $f \vdash_\mathcal{A}^M f'$ is a constant. We write $M = M_u + M_s$ with $ M_u$ the number of usual steps and $M_s$ the number of shift rule applications. When processing $C_{\tilde{f}}(n)$, each shift rule is confronted to $n$ repetitions of a repeater (because shift rules preserve the number of repetitions), hence we get: $g(n+1) - g(n) = M_u + n M_s$, which means $D(D((g(n))_{n\in\N})) = (M_s)_{n\in\N}$ which is constant, as needed.
...
\begin{example}\label{ex:linquad}
    Using Example~\ref{ex:bouncerTheory}, we get that $f = 0^\infty (111)1110(11)00\rhead{\text{D}}0^\infty$ solves the bouncer given in Example~\ref{ex:bouncer88427177}. We have $f \vdash_\mathcal{A}^{M} f'$ with $M = 41$ and $f'$ special case of $f$. We can decompose $M = M_u + M_s$ with $M_u = 39$ usual steps and $M_s = 2$ shift rule applications.

    Using Theorem~\ref{th:linquad}, we have $\text{linquad}(f)= 0^\infty (111)1111110(11)11 00\rhead{\text{D}}0^\infty$. Calling $g(n)$ the time step at which $C_{\text{linquad}(f)}(n)$ is reached, we have $g(0)= 64$. Using the proof of Theorem~\ref{th:linquad}, $g(n+1)-g(n) = M_u + n M_s = 39 + 2n$, hence, $g(n) = n^2 + 39n + 64$. We also have $l_{g(n)} = 11 + 5n$.
