...
\begin{theorem}[Greedy formula tape fitting]\label{th:greedy}
...
\begin{remark}[Minimal formula tapes]
    While, by Theorem~\ref{th:greedy}, Algorithm~\ref{alg:greedy-formula-tape-fitting} is assured to fit a formula tape (if it exists) starting from $t_0$ when it has at least one occurrence of each repeater, it can fail when $t_0$ has none. Here is an example trying to parse $1011(01011)(1)0$ starting from $t_0 = 10110$:
    \begin{align*}
        t_0 & = 1011\; 0                         \\
        t_0 & = 1011\; 01011\; 1\; 0             \\
        t_1 & = 1011\; 01011\; 01011\; 1\; 1\; 0
    \end{align*}
    The greedy algorithm will raise failure on the following configuration:
    \begin{align*}
        t'_0 & = \varnothing              \\
        t'_1 & = 1011\; 1\; 0             \\
    ...
    Hence, the greedy algorithm does not always fit \textit{minimal} formula tapes. By adding backtracking, it can easily be transformed into a Dynamic Programming algorithm that will fit minimal formula tapes, at the cost of being less efficient (quadratic vs. linear time complexity).
