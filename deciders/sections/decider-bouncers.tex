...
The goal of this section is to formally characterise bouncers and show that they do not halt, see Theorem~\ref{th:bouncers}. Then, in Section~\ref{sec:bouncers-implem}, we show how to detect bouncers algorithmically in practice, see Algorithm~\ref{alg:decider-bouncers}.
...
\begin{definition}[Bouncer certificate]\label{def:bouncer-certificate}
    ...
    \end{enumerate}
    Given such certificate, one can verify that the machine is a bouncer by applying Theorem~\ref{th:bouncers} or that the certificate is erroneous. Additional information, such as the list of used shift rules can be added to the certificate for convenience.
\end{definition}
...
\subsection{Implementations}\label{sec:bouncers-implem}
