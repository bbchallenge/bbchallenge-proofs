...
\begin{proof}
    Without loss of generality we suppose that $f$ is aligned. For simplicity of notations, we also suppose that there are no repeaters after the tape head in $f$, as we would apply exactly the same transformations after the head. Using Equation~\eqref{math:formulaTapes}, we write $f=w_1(r_1)w_2(r_2)\dots w_n(r_n) w_{n+1} h w$ with $n \geq 1$, $w_i \in \Sigma^*$ for $2 \leq i \leq n+1$, $w_1,w \in \overline{\Sigma}^*$ and $r_i \in \Sigma^+$ for $1 \leq i \leq n$ and $h\in\Delta$.
...
\begin{lemma}\label{lem:linquad-intermediary-walls}
    Let $f$ be a right-aligned formula tape with nonempty intermediary walls then $\text{linquad}(f)$ built in Theorem~\ref{th:linquad} is also right-aligned with nonempty intermediary walls.
\end{lemma}
\begin{proof}
    The construction of $\text{linquad}(f)$ only involves (1) removing some repeaters (2) replacing some repeaters by some powers of themselves (3) increasing the size of some walls. Hence, $\text{linquad}(f)$ has nonempty intermediary walls. As per alignment, the transformation increases walls to the left of repeaters with repeaters' repetitions, both before and after the head, this mean that right-alignment is preserved.
\end{proof}
...
\end{theorem}
\begin{proof}
    Because $M$ is a bouncer, using Lemma~\ref{lem:record-breaking} we know that there is a record-breaking formula tape $\tilde{f}$ that solves the bouncer. By Lemma~\ref{lem:formula-tapes-WLOG} we know that we can transform $\tilde{f}$ into $f$ that is a right-aligned with nonempty intermediary walls and that also solves the bouncer.

    Algorithm~\ref{alg:decider-bouncers} enumerates all record-breaking tapes triple that grow linearly in quadratic time. If $s$ is large enough, by Theorem~\ref{th:linquad}, we know that it will, given a large enough limit $l$ for testing formula tapes, eventually meet a triple of the form: \begin{align*}
        t_0 & = C_{\text{linquad}(f)}(0)  \\
        t_1 & = C_{\text{linquad}(f)}f(1) \\
        t_2 & = C_{\text{linquad}(f)}f(2)
    \end{align*}

    By Lemma~\ref{lem:linquad-intermediary-walls}, $f'=\text{linquad}(f)$ is right-aligned with nonempty intermediary walls and, at line 23, Algorithm~\ref{alg:decider-bouncers} will feed $(\mathcal{S}(C_{f'}(0)),\mathcal{S}(C_{f'}(1)),\mathcal{S}(C_{f'}(2)))$ to Algorithm~\ref{alg:greedy-formula-tape-fitting}, which, by Theorem~\ref{th:greedy-formula-tape-fitting}, will output $\mathcal{S}(\text{linquad}(f))$ which be reconstructed into $\text{linquad}(f)$ at line 27. By Theorem~\ref{th:linquad}, we know that $\text{linquad}(f)$ also solves the bouncer and Theorem~\ref{th:bouncers} will be verified on $\text{linquad}(f)$ using \textbf{reaches\_special\_case} at line 28, which, by hypothesis will output \textbf{true}, and we have the result.
\end{proof}
