...
\begin{example}\label{ex:linquad}
    ...
    Calling $g(n)$ the time step at which $C_{\text{linquad}(f)}(n)$ is reached, we have $g(0)= 64$. Using the proof of Theorem~\ref{th:linquad}, $g(n+1)-g(n) = M_u + n M'_s = 45 + 6n$, hence, $g(n) = 3n^2 + 42n + 64$. We also have $l_{g(n)} = 11 + 5n$. One can verify by simulation that tapes $C_{\text{linquad}(f)}(n)$ are reached at time steps $g(n)$.
