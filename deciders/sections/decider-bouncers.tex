...
\begin{lemma}\label{lem:right-alignement}
    Assume that $M$ is a bouncer solved by formula tape $f$ according to Theorem~\ref{th:bouncers}, then:
    \begin{enumerate}
        \item Any special case $f'$ of $f$ also solves the bouncer according to Theorem~\ref{th:bouncers}.
...
\begin{lemma}\label{lem:formula-tapes-WLOG}
    Let a machine $M$ be a bouncer, then there is a right-aligned formula tape with nonempty intermediary walls that solves the bouncer according to Theorem~\ref{th:bouncers}.
\end{lemma}
\begin{proof}
...
We finally present a complete decider for bouncers, Algorithm~\ref{alg:decider-bouncers} which is proven correct in Theorem~\ref{th:bouncer-decider}. The motivation for focusing on record-breaking formula tapes is that is considerably decreases the number of tapes and tapes triples to consider in the algorithm.
