...
\begin{theorem}[Greedy formula tape fitting]\label{th:greedy-formula-tape-fitting}
    Let there be 3 words $t_0, t_1, t_2 \in \Sigma^*$, an integer $m>0$ and nonempty intermediary walls $w_2, \dots w_m \in \Sigma^+$ and possibly-empty initial and final walls $\alpha, \beta \in \Sigma^*$ and nonempty repeaters $r_1, \dots, r_m \in\Sigma^+$ so that they are the longest repeaters for $t_0, t_1, t_2$ to be written as:
    \begin{align*}
        t_0 & = \alpha\; w_2 \dots w_m\; \beta                       \\
        t_1 & = \alpha\; r_1\; w_2\; \dots w_m\; r_m\; \beta         \\
        t_2 & = \alpha\; r_1 r_1\; w_2\; \dots w_m\; r_m r_m\; \beta
    \end{align*}
    Then Algorithm~\ref{alg:greedy-formula-tape-fitting} ran on {\sc Fit-FormulaTape}($t_0$, $t_1$, $t_2$) does not raise failure and returns the right-aligned headless formula tape $\mathcal{A}(\alpha (r_1) w_2 (r_2)\dots w_m (r_m) \beta)$.

\end{theorem}
\begin{proof}
    Let's first prove the case $m=1$ and write $r = r_1$.
    ...
    Two cases:
    \begin{itemize}
        \item If $r'$ is nonempty, we are in the previously seen case $(\beta,r\beta,rr\beta)$ by assimilating  $\beta'$ to $\beta$ and $ur'$ to $r$, except that case (3) cannot happen since we've chosen $\beta'$ and $r'$ such that $\beta'[0] \neq r'[0]$ if they are both nonempty. Hence, the outputted formula is $f = \alpha u (r' u) \beta'$  which is the right-aligned version of $\alpha(r)\beta$ (because $\beta'[0] \neq r'[0]$, if both nonempty), as needed.
        \item If $r'$ is empty, we are also in the previously seen case $(\beta,r\beta,rr\beta)$ by assimilating  $\beta'$ to $\beta$ and $u$ to $r$, however $|\beta'| < |\beta|$, hence, by induction, using the same set of 3 cases, this system will eventually reach case (1) $\beta' = \varnothing$ or case (2) $\beta'[0] = u[0]$ where we can conclude.
    \end{itemize}
