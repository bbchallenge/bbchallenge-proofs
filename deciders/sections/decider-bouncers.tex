...
Given a formula tape $f$ let $\mathcal{L}(f)$ denote the language described by $f$, i.e.\ the set of tapes that match the tape formula.
...
(right) \textit{shift rules}, rewrite rules for formula tapes, which rewrite a subword $u \rhead{s}(r)$ into $(\tilde{r})u\rhead{s}$, denoted $u \rhead{s}(r) \to (\tilde{r})u\rhead{s}$. Similarly, left shift rules are of the form $(r)\lhead{s}u \to \;\lhead{s}u(\tilde{r})$. Note that repeaters $r$ and $\tilde{r}$ of a shift rule have the same size because $u$ is present on both sides of the shift rules, which are of equal size (since applying $\vdash$ preserves the size of the tape).
...
From the above definition of $\vdash$ on a formula tape $f$, it is clear that (i) there is at most one formula tape $f'$ such that $f \vdash f'$ and (ii) for usual tapes our new definition of $\vdash$ coincides with the old one.
...
One last tool that we need before characterising bouncers and proving that they do not halt (Theorem~\ref{th:bouncers}) is formula tape \textit{alignment} which transforms a formula tape into an equivalent other formula tape for the sake of being able to apply shift rules.

\paragraph*{Aligning formula tapes.} We can motivate formula tape \textit{alignment} using the following example: take $f=0^\infty(111)1111\rhead{\text{B}}(01)01010110^\infty$ for the machine given in Example~\ref{ex:bouncer88427177}. One can verify that no right shift rule applies to $f$ hence $\vdash$ does not apply to $f$. However, take $f'=0^\infty(111)1111\rhead{B}010101(01)10^\infty$, we have $\mathcal{L}(f) = \mathcal{L}(f')$ but we can apply $\vdash$ to $f'$ by performing usual steps: $f'=0^\infty(111)1111\rhead{\text{B}}010101(01)10^\infty \vdash^{12} 0^\infty(111)1111110110 \rhead{D} (01)10^\infty$ and now the shift rule of Example~\ref{ex:shiftRules} can apply, giving $0^\infty(111)111111011(11)0 \rhead{\text{D}} 10^\infty$. Formula tape $f'$ is called the \textit{alignment} of $f$ and is denoted $f' = \mathcal{A}(f)$.
...
\begin{example}
    Take $f=0^\infty 101(11)0 \rhead{\text{D}} 10 (100) 11 0^\infty$, we have $\mathcal{A}(f) = 0^\infty 10(11)10 \rhead{\text{D}} 101(001)10^\infty$.
\end{example}

\begin{remark}
    The example used for motivating the alignment operator shows that if no shift rules applies to $f$, it is possible that a shift rule applies to $\mathcal{A}(f)$ after consuming all possible usual steps. One can show that if a shift rule applies to $f$ then it also applies to $\mathcal{A}(f)$ (after consuming all possible usual steps): using the alignment operator can only increase the number of applicable shift rules.
\end{remark}
