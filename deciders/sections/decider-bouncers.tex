...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        \State \textbf{struct} TapeTriple \{
        \State \tabi\textbf{Tape} t0, t1, t2
        \State \}
        \State
        \Procedure{\textbf{FormulaTape} {\sc Fit-FormulaTape}}{\textbf{uint} pos\_t0, \textbf{uint} total\_repeater\_size, \textbf{TapeTriple} t}
        ...
        \State
        \If{i0 == t.t0.\textbf{len}() \&\& i1 == t.t1.\textbf{len}()}
        \State \Return \textbf{FormulaTape}::\textbf{empty}()

        \EndIf
        \State
        \If{i0 $<$ t.t0.\textbf{len}() \&\& i1 $<$ t.t1.\textbf{len}() \&\& i2 $<$ t.t2.\textbf{len}() \\ \&\& t.t0[i0] == t.t1[i1] \&\& t.t1[i1] == t.t1[i2]}
        \State \Return \textbf{FormulaTape}::\textbf{symbol}(t.t0[i0]).\textbf{concat}(\Call{{\sc Fit-FormulaTape}}{i0+1, k, t})
        \EndIf
        \State
        \State
        \textbf{uint} longest\_match\_size = \textbf{get\_longest\_match\_size}(t.t1[i1:], t.t2[i2:])
        \State
        \For{l = longest\_match; k $\geq$ 1; k -= 1 }

        \If{i2+2*l $<$ t.t2.\textbf{len}() \&\& t.t2[i2:i2+l] == t.t2[i2+l:i2+2*l] }
        \State \Return \textbf{FormulaTape}::\textbf{repeater}(t.t2[i2:i2+l]).\textbf{concat}(\Call{{\sc Fit-FormulaTape}}{i0, k+l, t})
        \EndIf
...
\begin{theorem}[Greedy formula tape fitting]
    Let there be 3 tapes $t_0, t_1, t_2$, and an integer $m\in\N$ and $w_1, \dots w_{m+1} \in \Sigma^*$ such that $w_2,\dots, w_m$ are not empty and $r_1, \dots, r_m \in\Sigma^+$ such that:
    \begin{align*}
        \mathcal{S}(t_0) & = w_1\; r_1\; \dots w_m\; r_m\; w_{m+1}               \\
        \mathcal{S}(t_1) & = w_1\; r_1r_1\; \dots w_m\; r_m r_m\; w_{m+1}        \\
        \mathcal{S}(t_2) & = w_1\; r_1 r_1 r_1 \dots w_m\; r_m r_m r_m\; w_{m+1}
    \end{align*}
    Then Algorithm~\ref{alg:greedy-formula-tape-fitting} does not raise failure and returns the formula tape $\mathcal{S}(\mathcal{A}(f))$ with $\mathcal{S}(f) = w_1 r_1 (r_1) \dots w_m r_m (r_m) w_{m+1}$.
