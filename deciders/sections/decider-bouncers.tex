...
\begin{example}\label{ex:linquad}
    Using Example~\ref{ex:bouncerTheory}, we get that $f = 0^\infty (111)1110(11)00\rhead{\text{D}}0^\infty$ solves the bouncer given in Example~\ref{ex:bouncer88427177}. Using Theorem~\ref{th:linquad}, we have $\operatorname{linquad}(f)= 0^\infty (111)1111110(11)11 00\rhead{\text{D}}0^\infty$.
    We have $\operatorname{linquad}(f) \vdash_\mathcal{A}^{M} f'$ with $M = 47$ and $f'$ special case of $\operatorname{linquad}(f)$. Note that $47 > 41$ found in Example~\ref{ex:bouncerTheory} because $\operatorname{linquad}(f)$ is a bit bigger than $f$.
    ...
    Calling $g(n)$ the time step at which $C_{\operatorname{linquad}(f)}(n)$ is reached, we have $g(0)= 64$. Using the proof of Theorem~\ref{th:linquad}, $g(n+1)-g(n) = M_u + n M'_s = 45 + 6n$, hence, $g(n) = 3n^2 + 42n + 64$. We also have $l_{g(n)} = 11 + 5n$. One can check by simulation that tapes $C_{\operatorname{linquad}(f)}(n)$ are reached at time steps $g(n)$.
...
\begin{lemma}\label{lem:linquad-intermediary-walls}
    Let $f$ be a formula tape with nonempty intermediary walls then $\operatorname{linquad}(f)$ built in Theorem~\ref{th:linquad} has nonempty intermediary walls.
\end{lemma}
\begin{proof}
    The construction of $\operatorname{linquad}(f)$ only involves (1) removing some repeaters (2) replacing some repeaters by some powers of themselves (3) increasing the size of some walls. Hence, $\operatorname{linquad}(f)$ has nonempty intermediary walls.
\end{proof}
...
\begin{proof}
    ...
    Algorithm~\ref{alg:decider-bouncers} enumerates all record-breaking tapes triple that grow linearly in quadratic time. If $s$ and $l$ are large enough, by Theorem~\ref{th:linquad}, we know that it will, eventually meet a triple of the form: \begin{align*}
        t_0 & = C_{\operatorname{linquad}(f)}(0)  \\
        t_1 & = C_{\operatorname{linquad}(f)}f(1) \\
        t_2 & = C_{\operatorname{linquad}(f)}f(2)
    \end{align*}

    By Lemma~\ref{lem:linquad-intermediary-walls}, $f'=\operatorname{linquad}(f)$ is with nonempty intermediary walls and, at line 23, Algorithm~\ref{alg:decider-bouncers} will feed $(\mathcal{S}(C_{f'}(0)),\mathcal{S}(C_{f'}(1)),\mathcal{S}(C_{f'}(2)))$ to Algorithm~\ref{alg:greedy-formula-tape-fitting}, which, by Theorem~\ref{th:greedy-formula-tape-fitting}, will output $\mathcal{S}(\mathcal{A}_r(\operatorname{linquad}(f)))$ which be reconstructed into $\mathcal{A}_r(\operatorname{linquad}(f))$ at line 27. By Theorem~\ref{th:linquad}, we know that $\operatorname{linquad}(f)$ also solves the bouncer, and by Lemma~\ref{lem:right-alignement}, $\mathcal{A}_r(\operatorname{linquad}(f))$ also solves the bouncer. Theorem~\ref{th:bouncers} will be verified on $\mathcal{A}_r(\operatorname{linquad}(f))$ using \textbf{reaches\_special\_case} at line 28, which, by hypothesis will output \textbf{true}, and we have the result.
\end{proof}

\begin{remark}
    While Theorem~\ref{th:bouncer-decider} assures us that the decider will be able to detect a bouncer, if given enough time, the formula tapes found in practice by the algorithm can be smaller than the ones constructed using $\operatorname{linquad}(f)$ with nonempty intermediary walls in the proof.
\end{remark}
